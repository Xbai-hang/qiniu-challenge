...
class AiControllerTests {
        ...
        void conversationCanBeDeletedFromHistory() throws Exception {
                RegisteredUser user = register("ai_conversation_delete_user", "ai-conversation-delete-user@example.com",
                                "AI Delete History");
                ...
                mockMvc.perform(get("/api/ai/conversations")
                                ...
                                .andExpect(jsonPath("$.data[*].id",
                                                Matchers.not(Matchers.hasItem((int) conversationId))));
        ...
        void chatCanCreateEventAndUndoLastAiOperation() throws Exception {
                RegisteredUser user = register("ai_chat_create_user", "ai-chat-create-user@example.com",
                                "AI Chat Create");
                when(aiModelClient.chat(any()))
                                .thenReturn(new AiModelResponse("test-model", "test", "",
                                                List.of(new AiRequestedToolCall(
                                                                "call_create_event",
                                                                "create_event",
                                                                Map.of(
                                                                                "title", "项目复盘",
                                ...
                                .thenReturn(new AiModelResponse("test-model", "test", "",
                                                List.of(new AiRequestedToolCall(
                                                                ...
                                                                Map.of(
                                                                                ...
                                                                                "end", "2026-05-31T00:00:00+08:00")))))
                                .thenReturn(new AiModelResponse("test-model", "test",
                                                "当前范围内有 1 个日程：范围内日程，05-30 12:00 开始。", List.of()));
        ...
        void chatNormalizesAiToolTimesWithoutOffsetToShanghaiTimezone() throws Exception {
                RegisteredUser user = register("ai_chat_timezone_user", "ai-chat-timezone-user@example.com",
                                "AI Chat Timezone");
                when(aiModelClient.chat(any()))
                                .thenReturn(new AiModelResponse("test-model", "test", "",
                                                List.of(new AiRequestedToolCall(
        ...
        void confirmationCanExecuteHighRiskDeleteFromChat() throws Exception {
                RegisteredUser user = register("ai_chat_delete_user", "ai-chat-delete-user@example.com",
                                "AI Chat Delete");
        ...
        void chatReturnsAiServiceUnavailableWhenModelCallFails() throws Exception {
                RegisteredUser user = register("ai_chat_model_failed_user", "ai-chat-model-failed-user@example.com",
                                "AI Chat Failed");
                when(aiModelClient.chat(any()))
        ...
        void chatCanAnswerCasualConversationWithoutTools() throws Exception {
                RegisteredUser user = register("ai_chat_casual_user", "ai-chat-casual-user@example.com",
                                "AI Chat Casual");
                when(aiModelClient.chat(any()))
                                .thenReturn(new AiModelResponse("test-model", "test", "我是你的 AI 日历助手，也可以陪你聊聊天。",
                                                List.of()));
        ...
        void speechTranscribeReturnsMockTranscriptAndPersistsMetadata() throws Exception {
                RegisteredUser user = register("speech_transcribe_user", "speech-transcribe-user@example.com",
                                "Speech User");
        ...
        void speechTranscribeAndChatFeedsTranscriptIntoAiAgent() throws Exception {
                ...
                MatcherAssert.assertThat(
                                requestCaptor.getValue().messages().stream()
                                                .anyMatch(message -> "user".equals(message.role())
                                                                && message.content().contains("明天上午十点提醒我做项目复盘")),
                                Matchers.is(true));
        }
